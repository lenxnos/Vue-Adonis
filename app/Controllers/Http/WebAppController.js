...
class AppController {
  ...
  async render ({request, response, view}) {  
      if (!this.renderer) {
       ...
       await this.render({request, response, view});
      return;
    }
    // Call webpack middlewares only in development
    if (this.isDev) {
       await this._webpackMiddlewares(request, response)
    }
    ...
    try {
     const html =  await this.renderToString(context)
      ...
      // throw err
      return response.status(500).send(view.render('errors.vue', { err, ansiHTML, encodeHtml }))
    }
  }
  ...
  async _webpackMiddlewares (request, response) {

     await this.webpackDevMiddleware(request.request, response.response)
     await this.webpackHotMiddleware(request.request, response.response)
  }
